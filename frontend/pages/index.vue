...
onMounted(() => {
  autoRefreshTimer = setInterval(async () => {
    ...
    try {
      await refreshRound()
      setLatestRound(round.value)
      // REVIEW-MAJOR-3 + GF-STALE (2026-09-22, user report): during GF
      // week ALWAYS refresh the grand-final view — the static bake can
      // lag the backend (a regenerated report lands after the last
      // build), and a present-but-STALE report never re-fires the
      // `grand-final-view` watcher on its own.  One extra fetch per
      // poll keeps open tabs current.
      if (isGrandFinal.value) {
