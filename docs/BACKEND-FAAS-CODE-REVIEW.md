...
The README claims "Tip generation runs at 3 AM AWST" ([`README.md:136`](../backend/README.md:136)). The actual cron in [`project.yml:108`](../backend/project.yml:108) is `"0 19 * * *"` (19:00 UTC = 03:00 AWST). The config field [`config.py:53`](../backend/packages/shared/config.py:53) `cron_tip_generation: str = "0 3 * * *"` is **commented** as "3:00 AM daily" but is *never used* by the platform — the platform only reads `project.yml`.
