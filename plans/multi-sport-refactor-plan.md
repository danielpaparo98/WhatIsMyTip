...
| P0-7 | Dead code: `_check_cache` ×4; deprecated `GET /api/backtest/`; `generation_service.close()` no-op call; stray root `test_*.py` in `backend/` | 4 model files, `app/api/backtest.py`, `app/cron/tip_generation.py` | Removed; suite green; no references remain (grep-verified) |
