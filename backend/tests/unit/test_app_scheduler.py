...
class TestBuildScheduler:
    ...
    def test_league_sync_registered_when_enabled(self, monkeypatch):
        """The league-sync job is registered when ``league_sync_enabled`` is True."""
        from packages.shared import config as config_module

        fake_settings = config_module.Settings(league_sync_enabled=True)
        monkeypatch.setattr(scheduler_module, "settings", fake_settings)

        scheduler = build_scheduler(_make_session_factory())
        job = scheduler.get_job("league-sync")
        assert job is not None
        assert isinstance(job.trigger, CronTrigger)
        assert job.max_instances == 1
        assert job.coalesce is True

    def test_league_sync_not_registered_when_disabled(self, monkeypatch):
        """The league-sync job is absent when ``league_sync_enabled`` is False."""
        from packages.shared import config as config_module

        fake_settings = config_module.Settings(league_sync_enabled=False)
        monkeypatch.setattr(scheduler_module, "settings", fake_settings)

        scheduler = build_scheduler(_make_session_factory())
        assert scheduler.get_job("league-sync") is None
        # The other jobs are still registered.
        job_ids = {job.id for job in scheduler.get_jobs()}
        assert {"daily-sync", "match-completion", "tip-generation", "historic-refresh"} <= job_ids

    def test_league_sync_uses_cron_string_from_settings(self, monkeypatch):
        """A custom ``league_sync_cron`` expression is plumbed into the trigger."""
        from packages.shared import config as config_module

        fake_settings = config_module.Settings(
            league_sync_cron="15 4 * * *", league_sync_enabled=True
        )
        monkeypatch.setattr(scheduler_module, "settings", fake_settings)
