...
from .tip_generation import TipGenerationService
...
class HistoricDataRefreshService:
        ...
        try:
            ...
            try:
                ...
                for i, season in enumerate(self.seasons):
                    ...
                    try:
                        self.logger.info(
                            f"Processing season {season} ({i + 1}/{len(self.seasons)})"
                        )

                        # Sync games for this season
                        self.logger.info(f"Syncing games for season {season}")
                        sync_service = GameSyncService(
                            squiggle_client=squiggle_client, db_session=self.db, season=season
                        )

                        sync_result = await sync_service.sync_games()
                        season_stats["games_synced"] = (
                            sync_result["games_created"] + sync_result["games_updated"]
                        )
                        stats["games_synced"] += season_stats["games_synced"]

                        self.logger.info(
                            f"Season {season}: Synced {season_stats['games_synced']} games "
                            f"({sync_result['games_created']} created, "
                            f"{sync_result['games_updated']} updated, "
                        ...
                        if self.round_id:
                            ...
                            self.logger.info(
                                f"Generating tips for season {season}, round {self.round_id}"
                            )
                            tip_service = TipGenerationService(
                                db_session=self.db, season=season, round_id=self.round_id
                            )
                            ...
                            season_stats["tips_generated"] = (
                                tip_result["tips_created"] + tip_result["tips_updated"]
                            )
                            ...
                            tip_service = TipGenerationService(db_session=self.db, season=season)
                            ...
                            for round_id in rounds:
                                try:
                                    ...
                                    error_msg = (
                                        f"Error generating tips for season "
                                        f"{season}, round {round_id}: {str(e)}"
                                    )
    ...
    async def refresh_from_string(
        self, seasons_str: str, round_id: Optional[int] = None, regenerate_tips: bool = False
    ) -> Dict[str, Any]:
    ...
    async def get_progress(self) -> Optional[Dict[str, Any]]:
        ...
        if not self.progress_id:
            ...
            progress = await GenerationProgressCRUD.get_by_operation(
                db=self.db, operation_type="historic_refresh"
            )
            if progress:
                self.progress_id = progress.id
            else:
        ...
        progress = await GenerationProgressCRUD.get_by_id(db=self.db, progress_id=self.progress_id)
