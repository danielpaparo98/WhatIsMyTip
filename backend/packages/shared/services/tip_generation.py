...
class TipGenerationService:
    ...
    def __init__(
        self, db_session: AsyncSession, season: Optional[int] = None, round_id: Optional[int] = None
    ):
    ...
    async def generate_for_round(
        ...
        self.logger.info(
            f"Starting tip generation for season {season}, "
            f"round {round_id}, regenerate={regenerate}, skip_nlp={skip_nlp}"
        )
        ...
        try:
            ...
            for game in games:
                try:
                    ...
                    stats["model_predictions_created"] += game_stats.get(
                        "model_predictions_created", 0
                    )
                    stats["model_predictions_updated"] += game_stats.get(
                        "model_predictions_updated", 0
                    )
                    ...
                    error_msg = (
                        f"Error processing game {game.id} "
                        f"({game.home_team} vs {game.away_team}): {str(e)}"
                    )
    ...
    async def _generate_for_game(
        self, game: Game, regenerate: bool = False, skip_nlp: bool = False
    ) -> Dict[str, Any]:
        ...
        for heuristic in heuristics_to_use:
            ...
            try:
                ...
                self.logger.error(
                    f"Error generating {heuristic} tip for game {game.id}: {str(e)}", exc_info=True
                )
                raise
        ...
        if not skip_nlp and game_stats["tips_created"] > 0:
            try:
                ...
                analysis = await match_analysis_service.generate_and_store_analysis(self.db, game)
                if analysis:
                    self.logger.info(f"Generated match analysis for game {game.id}")
