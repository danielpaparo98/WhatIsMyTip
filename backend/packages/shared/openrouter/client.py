...
class OpenRouterClient:
    ...
    async def generate_explanation(
        ...
        if not settings.openrouter_api_key:
            ...
            return self._generate_fallback_explanation(game, prediction, heuristic)

        # Build context for the AI
        context = self._build_prompt_context(game, prediction, heuristic, model_predictions)
...
"""
    ...
    def _generate_fallback_explanation(self, game: dict, prediction: dict, heuristic: str) -> str:
        ...
        if heuristic == "best_bet":
            return (
                f"{winner} is the consensus pick across multiple models. "
                f"Confidence is {confidence:.0%} with a predicted "
                f"margin of {margin} points."
            )
        elif heuristic == "yolo":
            return (
                f"Going all in on {winner} with {confidence:.0%} confidence. "
                f"The highest confidence model predicts a {margin}-point win."
            )
        else:  # high_risk_high_reward
            return (
                f"Targeting an upset with {winner}. Models are split, "
                f"creating a high-risk high-reward opportunity. "
                f"Predicted margin: {margin} points."
            )
