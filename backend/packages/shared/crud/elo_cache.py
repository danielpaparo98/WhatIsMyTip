...
class EloCacheCRUD:
    ...
    async def save_ratings(
        ...
        for team_name, rating in ratings.items():
            ...
            result = await db.execute(select(EloCache).where(EloCache.team_name == team_name))
            cache_entry = result.scalar_one_or_none()

            if cache_entry:
                # Update existing entry
                cache_entry.rating = rating
                cache_entry.games_played = (
                    games_played.get(team_name, 0) if games_played else cache_entry.games_played
                )
    ...
    async def load_ratings(db: AsyncSession, season: Optional[int] = None) -> Dict[str, float]:
        ...
        if season is not None:
            ...
            result = await db.execute(select(EloCache).where(EloCache.season == season))
        else:
    ...
    async def load_ratings_with_games_played(
        db: AsyncSession, season: Optional[int] = None
        ...
        logger.info(
            f"Loaded {len(ratings)} Elo ratings with games played "
            f"from cache (season: {season or 'all'})"
        )
    ...
    async def clear_cache(db: AsyncSession, season: Optional[int] = None) -> int:
        ...
        if season is not None:
            ...
            count_result = await db.execute(select(func.count()).select_from(EloCache))
    ...
    async def get_team_rating(
        db: AsyncSession, team_name: str, season: Optional[int] = None
    ) -> Optional[float]:
