"""Show existing tips for a round from the database (no regeneration)."""
import asyncio
import os
import sys
...
async def main():
    ...
    async with SessionLocal() as db:
        ...
        print(f"\nGames in Round {round_id}, Season {season}:")
        for g in games:
        ...
        for game_key, data in games_map.items():
            ...
            print(f"  {'Model':<22} {'Winner':<28} {'Confidence':>12} {'Margin':>10}")
            print(f"  {'-'*22} {'-'*28} {'-'*12} {'-'*10}")
            for model_name in sorted(preds.keys()):
                ...
                print(f"  {model_name:<22} {p['winner']:<28} {conf_str:>12} {margin_str:>10}")
        ...
        for tip, game in tips:
            ...
            print(f"  {'Heuristic':<25} {'Selected':<28} {'Confidence':>12} {'Margin':>10}")
            print(f"  {'-'*25} {'-'*28} {'-'*12} {'-'*10}")
            conf_str = f"{tip.confidence:.1%}" if tip.confidence else "N/A"
            margin_str = f"{tip.margin}pts" if tip.margin else "N/A"
            print(f"  {tip.heuristic:<25} {tip.selected_team:<28} {conf_str:>12} {margin_str:>10}")
